...
assert(widgets.itv2TrackAllButton.text=='TRACK_ALL','部分追蹤時應顯示全部加入')
Click('itv2Down1')
...
assert(widgets.itv2TrackAllButton.text=='UNTRACK_ALL','全部追蹤時應顯示全部移除')
Click('itv2TrackAllButton')
assert(widgets.itv2EditorSummary.text=='0/7','全部移除未清空本頁追蹤')
assert(widgets.itv2TrackAllButton.text=='TRACK_ALL','全部移除後應切回全部加入')
assert(S.IsTracked('GR'),'全部移除不應影響其他分類')
assert(widgets.itv2PopEmpty.visible,'全部移除後懸浮窗應顯示空清單提示')
S.Load()
assert(not S.IsTracked('AS_1') and not S.IsTracked('AS_7'),'全部移除結果未存檔')
Click('itv2TrackAllButton')
assert(widgets.itv2EditorSummary.text=='7/7','清空後無法重新全部加入')
for index,cat in ipairs(ITV2.CATEGORIES) do
    Click('itv2Tab'..index)
